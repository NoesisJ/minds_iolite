...
const searchCategories = (event: { query: string }) => {
  ...
  filteredCategories.value = uniqueCategories.filter(cat => cat.toLowerCase().includes(query));
};

const searchLocations = (event: { query: string }) => {
  const query = event.query.toLowerCase();
  const uniqueLocations = [...new Set(items.value.map(item => item.location))];
  filteredLocations.value = uniqueLocations.filter(loc => loc.toLowerCase().includes(query));
};
