<template>
  <div class="viewer-view">
    <!-- 顶部导航 -->
    <div class="viewer-header flex justify-between items-center p-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center space-x-4">
        <h1 class="text-xl font-semibold text-gray-800 dark:text-white">{{ currentPage?.title || '页面阅览' }}</h1>
    ...
    <!-- 页面内容 -->
    <div class="viewer-content bg-gray-50 dark:bg-gray-900 min-h-[calc(100vh-56px)] p-4">
      <div v-if="currentPage" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 max-w-6xl mx-auto">
        <div v-for="(region, index) in currentPage.regions" :key="index" class="mb-4">
          ...
          <div v-else class="empty-region p-4 text-center text-gray-400 border border-dashed border-gray-200 dark:border-gray-700 rounded">
            {{ region.name }} - 空区域
          </div>
        </div>
      </div>
      <div v-else class="flex flex-col items-center justify-center h-[60vh] text-gray-500 dark:text-gray-400">
        <i class="pi pi-exclamation-circle text-5xl mb-4"></i>
        <div class="text-xl mb-2">未找到页面</div>
        <p class="mb-4">请返回设计器创建或选择页面</p>
        <base-button status="primary" @click="returnToDesigner">返回设计器</base-button>
      </div>
    </div>
  </div>
</template>
