...
export const useDesignerStore = defineStore("designer", {
        ...
        regions: [
          {
            id: "header",
            name: "头部",
            allowedComponents: ["title", "search", "filter"],
          },
        ...
        regions: [
          {
            id: "header",
            name: "头部",
            allowedComponents: ["title", "filter"],
          },
          { id: "left", name: "左侧", allowedComponents: ["chart", "card"] },
          {
            id: "right",
            name: "右侧",
            allowedComponents: ["chart", "list", "card"],
          },
        ],
      },
      {
  ...
  actions: {
    ...
    moveComponent(
      ...
      // 查找源区域和目标区域
      const sourceRegion = this.pages[pageIndex].regions.find(
        (r) => r.id === sourceRegionId
      );
      const targetRegion = this.pages[pageIndex].regions.find(
        (r) => r.id === targetRegionId
      );
      if (!sourceRegion || !targetRegion) return;

      // 查找组件
      const componentIndex = sourceRegion.components.findIndex(
        (c) => c.id === componentId
      );
    ...
    deleteComponent(componentId: string) {
      ...
      for (const region of this.pages[pageIndex].regions) {
        const componentIndex = region.components.findIndex(
          (c) => c.id === componentId
        );
        if (componentIndex !== -1) {
      ...
      try {
        ...
        if (storedData) {
          ...
          if (
            ...
            this.pages.some((p) => p.id === parsedData.currentPageId)
          ) {
