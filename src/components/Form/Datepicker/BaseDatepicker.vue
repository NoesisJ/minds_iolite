      ...
      >
        ...
        <div class="mt-2 flex justify-between">
          ...
          <div>
            ...
            <button
              @click="applyPicker"
              class="text-sm px-2 py-1 bg-blue-500 text-white hover:bg-blue-600 rounded"
            >
              确定
            </button>
          </div>
...
const props = defineProps({
  ...
  status: {
    ...
    default: "",
    validator: (val) =>
      ["", "primary", "info", "success", "warning", "danger"].includes(val),
  },
});
...
const tempSelectedDate = ref(
  selectedDate.value ? new Date(selectedDate.value) : null
);
...
const isSelected = (day) => {
  ...
  return (
    tempSelectedDate.value.getDate() === day.day &&
...
const handleClickOutside = (event) => {
  ...
  // 检查点击是否在当前日期选择器组件内
  const isClickInside =
    pickerContainerRef.value?.contains(event.target) ||
