<template>
  <div class="echarts-interactive-pie-line-widget">
    ...
    <div ref="chartRef" :style="{ height: height || '450px' }" class="chart-container"></div>
  </div>
</template>
...
function updateChartOption() {
  ...
  // 使用传入的数据或默认数据
  const data = props.chartData && props.chartData.length > 0 
    ? props.chartData 
    : defaultData;
  ...
  const option = {
    ...
    series: [
      // 这里需要根据数据源动态创建折线系列
      ...Array(data.length - 1).fill(null).map(() => ({
        type: 'line',
        smooth: true,
        seriesLayoutBy: 'row',
